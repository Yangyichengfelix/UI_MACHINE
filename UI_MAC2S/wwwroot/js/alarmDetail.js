function cleanAlarmDetailChartChildren() {
    const alarm_detail_chart = document.getElementById("alarm_detail_chart");
    while (alarm_detail_chart.firstChild) {
        alarm_detail_chart.removeChild(alarm_detail_chart.lastChild);
    }
} 
function drawAlarmDetailChart(nokData, skewingData, seconds, start, end) {
    ...
    const alarmTooltip = d3.select("#alarm_detail_tooltip")
        .style("opacity", 0)
        .attr("class", "tooltip")
    ...
    const parent = d3.select("#alarm_detail_chart");
