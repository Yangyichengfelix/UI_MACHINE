function drawStatusDetailChart(data) {   
    ...
    const margin = {
        ...
        p = document.getElementById('statusDetail_chart').parentNode.clientWidth
        width=p
    height = 600,
        ...
        ;
    const radius = Math.min(width, height) / 3 - margin.top
    ...
    const HeightOffset = height /1.9
    const WidthOffset=width/1.5
    const tooltip = d3.select('#statusDetail_tooltip');
    ...
    const svgPie = d3.select('#statusDetail_chart').attr('width', width).attr('height', height - margin.top);
    ...
    slice
        ...
        .on('mouseover', (event, v) => {
            ...
            tooltip
                ...
                .style("opacity", 1)
                .html(`${v.data.name}, <span>Lasts about</span> ${time}, <span>Ratio</span>: ${ratio}%`)
                ...
                .style("top", (event.y) / 5 + "px").transition()
                .duration(duration)
        })
        .on('mouseout', (event, v) => {
            d3.select(event.currentTarget)
    ...
    polyline
        ...
        .attr('points', function (d, i) {
            ...
            const posC = outerLineArc.centroid(d);  // Label position = almost the same as posB

            const midangle = d.startAngle + (d.endAngle - d.startAngle) / 2 // we need the angle to see if the X position will be at the extreme right or extreme left
            // posB[0]=(i%2==0? posB[0]=20:posB[0])
            posB[0] = (ratio > 40 ? posA[0] : posB[0])
            posB[1] = (ratio > 40 ? posA[1] : posB[0])
            posC[0] = radius * 0.8 * (d.midangle > 6.5 ? 1.02 : -1.05); // multiply by 1 or -1 to put it on the right or on the left
            //posC[0]=  radius+i*25
    ...
    CodeLabels
        ...
        .attr('transform', function (d, i) {
            const pos = outerLineArc.centroid(d);
...
function cleanStatusDetailChartChildren() {
    const status_chart = document.getElementById("statusDetail_chart");
